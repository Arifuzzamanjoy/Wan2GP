...
def get_models_cache_path():
    """Get the configurable models cache path from server config"""
    # Use global server_config if available, otherwise default to ckpts
    try:
        cache_path = server_config.get("models_cache_path", "ckpts")
    except NameError:
        cache_path = "ckpts"
...
def get_wan_models_cache_path():
    """Get the cache path specifically for WAN models (safetensors)"""
    # Use global server_config if available, otherwise default to /home/caches
    try:
        wan_cache_path = server_config.get("models_cache_path", "/home/caches")
    except NameError:
        wan_cache_path = "/home/caches"
    
    if not os.path.exists(wan_cache_path):
        os.makedirs(wan_cache_path, exist_ok=True)
    return wan_cache_path

def migrate_wan_models_to_cache():
    """Migrate WAN model safetensors from ckpts/ to /home/caches"""
    old_path = "ckpts"
    new_path = get_wan_models_cache_path()
    
    if old_path == new_path or not os.path.exists(old_path):
        return
        
    print(f"Migrating WAN model safetensors from {old_path} to {new_path}...")
    
    # Create new directory if it doesn't exist
    os.makedirs(new_path, exist_ok=True)
    
    # Look for .safetensors files that contain "wan" in the name
    if os.path.isdir(old_path):
        for item in os.listdir(old_path):
            if item.endswith('.safetensors') and 'wan' in item.lower():
                ...
                try:
                    ...
                    print(f"Moved WAN model: {item}")
                    
                    # Create symlink back to original location for compatibility
                    os.symlink(new_item_path, old_item_path)
                    print(f"Created symlink: {old_item_path} -> {new_item_path}")
                    
                except Exception as e:
                    print(f"Error moving {item}: {e}")

def migrate_models_to_cache():
    """Migrate existing models from ckpts/ to the configured cache directory"""
    old_path = "ckpts"
    new_path = get_models_cache_path()
    
    if old_path == new_path:
        return  # No migration needed
        
    if os.path.exists(old_path) and os.path.isdir(old_path) and not os.path.islink(old_path):
        print(f"Migrating models from {old_path} to {new_path}...")
        ...
        # Remove old directory if empty, then create symlink
        try:
...
if not Path(server_config_filename).is_file():
    server_config = {
        ...
        "models_cache_path": "/home/caches",  # New: configurable models cache directory
        "compile" : "",
    ...
    # Add models_cache_path if it doesn't exist in existing config
    if "models_cache_path" not in server_config:
        server_config["models_cache_path"] = "/home/caches"
        with open(server_config_filename, "w", encoding="utf-8") as writer:
            writer.write(json.dumps(server_config))

# Call migration after server config is loaded
migrate_wan_models_to_cache()

# Setup model directories and migrate old files
if not os.path.isdir(os.path.join(get_models_cache_path(), "umt5-xxl")):
    os.makedirs(os.path.join(get_models_cache_path(), "umt5-xxl"))
cache_path = get_models_cache_path()
src_move = [ f"{cache_path}/models_clip_open-clip-xlm-roberta-large-vit-huge-14-bf16.safetensors", f"{cache_path}/models_t5_umt5-xxl-enc-bf16.safetensors", f"{cache_path}/models_t5_umt5-xxl-enc-quanto_int8.safetensors" ]
tgt_move = [ f"{cache_path}/xlm-roberta-large/", f"{cache_path}/umt5-xxl/", f"{cache_path}/umt5-xxl/"]
...
"ltxv_0.9.7_13B_dev_bf16.safetensors"
    ...
    # Check both regular cache and WAN cache for deprecated models
    cache_paths = [get_models_cache_path(), get_wan_models_cache_path()]
    for cache_path in cache_paths:
        file_path = os.path.join(cache_path, path)
        if Path(file_path).is_file():
            print(f"Removing old version of model '{path}' from {cache_path}. A new version of this model will be downloaded next time you use it.")
            os.remove(file_path)

for f, s in [(os.path.join(get_models_cache_path(), "Florence2/modeling_florence2.py"), 127287)]:
    try:
...
def get_model_filename(model_type, quantization ="int8", dtype_policy = "", module_type = None, submodel_no = 1, stack=[]):
    ...
    # Use WAN cache path for WAN models (safetensors), regular cache for others
    def get_cache_path_for_model(path, model_type):
        if path.startswith("http"):
            filename = os.path.basename(path)
            # Check if this is a WAN model safetensors file
            if filename.endswith('.safetensors') and ('wan' in filename.lower() or get_model_family(model_type) == "wan"):
                return os.path.join(get_wan_models_cache_path(), filename)
            else:
                return os.path.join(get_models_cache_path(), filename)
        return path
    
    choices = [ get_cache_path_for_model(path, model_type) for path in URLs ]
...
def save_model(model, model_type, dtype,  config_file,  submodel_no = 1,  is_module = False, filter = None, no_fp16_main_model = True ):
    ...
    # Use WAN cache for WAN models, regular cache for others
    if model_filename.endswith('.safetensors') and ('wan' in model_filename.lower() or get_model_family(model_type) == "wan"):
        model_filename = os.path.join(get_wan_models_cache_path(), model_filename)
    else:
        model_filename = os.path.join(get_models_cache_path(), model_filename)
...
def process_files_def(repoId, sourceFolderList, fileList):
    targetRoot = get_models_cache_path() + "/" 
...
def download_models(model_filename = None, model_type= None, module_type = None, submodel_no = 1):
    ...
    def download_file(url,filename):
        if url.startswith("https://huggingface.co/") and "/resolve/main/" in url:
            ...
            if len(sourceFolder) == 0:
                # Determine cache path based on file type and content
                cache_path = get_models_cache_path()
                if onefile.endswith('.safetensors') and 'wan' in onefile.lower():
                    cache_path = get_wan_models_cache_path()
                hf_hub_download(repo_id=repoId,  filename=onefile, local_dir = cache_path if len(base_dir)==0 else base_dir)
            else:
                # For files in subfolders, use regular cache path
                target_path = os.path.join(get_models_cache_path(), "temp", sourceFolder)
                if not os.path.exists(target_path):
                    os.makedirs(target_path)
                hf_hub_download(repo_id=repoId,  filename=onefile, local_dir = os.path.join(get_models_cache_path(), "temp"), subfolder=sourceFolder)
                
                # Determine final destination based on file type
                final_cache_path = get_models_cache_path()
                if onefile.endswith('.safetensors') and 'wan' in onefile.lower():
                    final_cache_path = get_wan_models_cache_path()
                
                shutil.move(os.path.join(get_models_cache_path(), "temp", sourceFolder, onefile), final_cache_path if len(base_dir)==0 else base_dir)
                shutil.rmtree(os.path.join(get_models_cache_path(), "temp"))
        else:
    ...
    if source is not None and module_type is None or module_source is not None and module_type is not None:
        ...
        for url in preload_URLs:
            ...
            # Use WAN cache for WAN model files
            if filename.endswith('.safetensors') and 'wan' in filename.lower():
                full_filename = os.path.join(get_wan_models_cache_path(), filename)
            else:
                full_filename = os.path.join(get_models_cache_path(), filename)
            if not os.path.isfile(full_filename): 
                ...
                try:
                    ...
                    if os.path.isfile(full_filename): os.remove(full_filename) 
...
def setup_prompt_enhancer(pipe, kwargs):
    ...
    if model_no != 0:
        ...
        prompt_enhancer_image_caption_model = AutoModelForCausalLM.from_pretrained( os.path.join(get_models_cache_path(), "Florence2"), trust_remote_code=True)
        prompt_enhancer_image_caption_processor = AutoProcessor.from_pretrained( os.path.join(get_models_cache_path(), "Florence2"), trust_remote_code=True)
        ...
        if model_no == 1:
            budget = 5000
            prompt_enhancer_llm_model = offload.fast_load_transformers_model(os.path.join(get_models_cache_path(), "Llama3_2/Llama3_2_quanto_bf16_int8.safetensors"))
            prompt_enhancer_llm_tokenizer = AutoTokenizer.from_pretrained(os.path.join(get_models_cache_path(), "Llama3_2"))
        else:
            budget = 10000
            prompt_enhancer_llm_model = offload.fast_load_transformers_model(os.path.join(get_models_cache_path(), "llama-joycaption-beta-one-hf-llava/llama_joycaption_quanto_bf16_int8.safetensors"))
            prompt_enhancer_llm_tokenizer = AutoTokenizer.from_pretrained(os.path.join(get_models_cache_path(), "llama-joycaption-beta-one-hf-llava"))
...
# Queue functionality removed - stub functions
...
def finalize_generation_with_state(state):
    return gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), state
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            if not update_form:
                ...
                # Hidden placeholder components for removed queue functionality
                queue_df = gr.DataFrame(visible=False)
                save_queue_btn = gr.Button(visible=False)
                load_queue_btn = gr.UploadButton(visible=False)
                clear_queue_btn = gr.Button(visible=False)
                queue_accordion = gr.Accordion("Queue", visible=False)
        ...
        if update_form:
            ...
            # Queue functionality removed - event handlers cleaned up
            ...
            output_trigger.change(refresh_gallery,
                inputs = [state], 
                outputs = [output, gen_info, generate_btn, add_to_queue_btn, current_gen_column, current_gen_buttons_row, abort_btn, onemorewindow_btn])
            ...
            # Queue functionality removed - handlers simplified and commented out
            
            # The following event handlers reference undefined UI components that were removed
            ...
            # Queue functionality removed - save/clear/add handlers removed
